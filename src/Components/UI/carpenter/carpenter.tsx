...
export const Carpenter:FC = () => {
  ...
  const handleCommentsChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
	...
	const submit = async () =>{
		...
		console.log(data);
		try {
			await axios.post(`https://sheet.best/api/sheets/662fb1f9-ba97-4d6c-9ec5-e188d69bfc95/tabs/carpenter`,data)
			alert('Вы записались к плотнику в ' + data.time + ' ' + data.date);
